    ...
    def colour_image_callback(self, msg):
        try:
            ...
            if self.depth_image is not None:
                ...
                processed_msg = self.bridge.cv2_to_imgmsg(processed_image, 'bgra8')  # Change to 'bgra8'
        ...
        for contour in contours:
            ...
            if len(approx) == 4 and depth > 0:
                ...
                if 0.95 <= aspect_ratio <= 1.05:
                    ...
                    color_image_bgra[mask_cube[:, :, 3] == 255] = [0, 255, 0, 128]  # Apply semi-transparent green color
                    
                    # Draw the contour in a different color to highlight the edges
                    cv2.drawContours(color_image_bgra, [approx], -1, color, 2)
                    x, y = approx[0][0]
                    cv2.putText(color_image_bgra, f"{shape}, Depth: {depth:.2f}mm", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
